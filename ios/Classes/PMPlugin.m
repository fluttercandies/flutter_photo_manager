...
- (void)initNotificationManager:(NSObject<FlutterPluginRegistrar> *)registrar {
  self.notificationManager =
      [PMNotificationManager managerWithRegistrar:registrar];
}
...
- (void)replyPermssionResult:(ResultHandler *)handler
    ...
    [PHPhotoLibrary
        requestAuthorizationForAccessLevel:requestAccessLevel
                                   handler:^(PHAuthorizationStatus status) {
                                     [self replyPermssionResult:handler
                                                         status:status];
                                   }];
  } else {
    [PHPhotoLibrary requestAuthorization:^(PHAuthorizationStatus status) {
...
- (void)requestPermissionStatus:(int)requestAccessLevel
                completeHandler:
                    (void (^)(PHAuthorizationStatus status))completeHandler {
  if (@available(iOS 14, *)) {
    [PHPhotoLibrary
        requestAuthorizationForAccessLevel:requestAccessLevel
                                   handler:^(PHAuthorizationStatus status) {
...
- (void)presentLimited:(ResultHandler *)handler {
  if (@available(iOS 15, *)) {
    ...
    if (!controller) {
      [handler reply:[FlutterError errorWithCode:@"UIViewController is nil"
                                         message:@"presentLimited require a "
                                                 @"valid UIViewController."
                                         details:nil]];
      return;
    }
...
- (void)onAuth:(FlutterMethodCall *)call result:(FlutterResult)result {
  ...
  [self runInBackground:^{
    if ([call.method isEqualToString:@"getGalleryList"]) {
      ...
      PMFilterOptionGroup *option = [PMConvertUtils
          convertMapToOptionContainer:call.arguments[@"option"]];
      ...
      PMProgressHandler *progressHandler =
          [self getProgressHandlerFromDict:call.arguments];
      ...
      [manager saveImage:data
                   ...
                   block:^(PMAssetEntity *asset) {
                     ...
                     NSDictionary *resultData =
                         [PMConvertUtils convertPMAssetToMap:asset
                                                   needTitle:NO];
      ...
      NSDictionary *resultMap = [PMConvertUtils convertPMAssetToMap:entity
                                                          needTitle:YES];
      ...
      PMFilterOptionGroup *option =
          [PMConvertUtils convertMapToOptionContainer:optionMap];

      NSArray<PMAssetPathEntity *> *array = [manager getSubPathWithId:galleryId
                                                                 type:type
                                                            albumType:albumType
                                                               option:option];
      ...
      NSString *galleryId = call.arguments[@"galleryId"];
      [manager copyAssetWithId:assetId
                     toGallery:galleryId
                         block:^(PMAssetEntity *entity, NSString *msg) {
                           if (msg) {
                             NSLog(@"copy asset error, cause by : %@", msg);
                             [handler reply:nil];
                           } else {
                             [handler reply:[PMConvertUtils
                                                convertPMAssetToMap:entity
                                                          needTitle:NO]];
                           }
                         }];
      ...
      [self
          requestPermissionStatus:2
                  completeHandler:^(PHAuthorizationStatus status) {
                        ...
                        reply:
                            [FlutterError
                                errorWithCode:@"PERMISSION_NOT_AUTHORIZED"
                                      message:@"fetchPathProperties only works "
      ...
      [self requestPermissionStatus:2
                    completeHandler:^(PHAuthorizationStatus status) {
                      if (status == PHAuthorizationStatusAuthorized) {
                        [self createAlbum:call manager:manager handler:handler];
                        return;
                      ...
                      [handler
                          reply:[FlutterError
                                    errorWithCode:@"PERMISSION_NOT_AUTHORIZED"
                                          message:
                                              @"fetchPathProperties only works "
                                              @"with authorized permission."
                                          details:nil]];
                    }];
      ...
      [manager removeInAlbumWithAssetId:assetId
                                albumId:pathId
                                  block:^(NSString *msg) {
                                    if (msg) {
      ...
      NSString *id = call.arguments[@"id"];
      int type = [call.arguments[@"type"] intValue];
      [manager removeCollectionWithId:id
                                 type:type
                                block:^(NSString *msg) {
                                  if (msg) {
      ...
      PMThumbLoadOption *option =
          [PMThumbLoadOption optionDict:call.arguments[@"option"]];
...
- (void)createFolder:(FlutterMethodCall *)call
  ...
  [manager createFolderWithName:name
                       parentId:parentId
                          block:^(NSString *id, NSString *errorMsg) {
                            [handler reply:[self convertToResult:id
                                                        errorMsg:errorMsg]];
                          }];
}

- (void)createAlbum:(FlutterMethodCall *)call
            manager:(PMManager *)manager
            handler:(ResultHandler *)handler {
  NSString *name = call.arguments[@"name"];
  BOOL isRoot = [call.arguments[@"isRoot"] boolValue];
  ...
  [manager createAlbumWithName:name
                      parentId:parentId
                         block:^(NSString *id, NSString *errorMsg) {
                           [handler reply:[self convertToResult:id
                                                       errorMsg:errorMsg]];
