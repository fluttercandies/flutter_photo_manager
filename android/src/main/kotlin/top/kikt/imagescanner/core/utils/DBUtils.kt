...
object DBUtils : IDBUtils {
  ...
  override fun moveToGallery(context: Context, assetId: String, galleryId: String): AssetEntity? {
    ...
    val (currentGalleryId, targetParent) = getSomeInfo(context, assetId)
    ...
    val cursor = cr.query(allUri, arrayOf(MediaStore.MediaColumns.DATA), idSelection, arrayOf(assetId), null) ?: throwMsg("Cannot find $assetId path")
    
    val targetPath = if (cursor.moveToNext()) {
      val srcPath = cursor.getString(0)
      cursor.close()
      val target = "$targetParent/${File(srcPath).name}"
